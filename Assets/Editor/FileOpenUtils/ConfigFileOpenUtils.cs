...
public static class ConfigFileOpenUtils {
    ...
    public static bool VerifyConfigFile(string path) {
        var validPath = path.Contains("Assets/Res/Data") && (path.ToLower().EndsWith(".txt") || path.ToLower().EndsWith(".lua"));
        return validPath;
    }
    ...
    public static void StartOpenConfigFile(string path) {
        ...
        if (openedFiles.Contains(excelFilePath)) {
            EditorUtility.DisplayDialog("警告", "文件打开中...", "确认");
            return;
        }
    ...
    public static string ReadAllText(string relativePath) {
        ...
        if (Application.isEditor) {
            path = Path.Combine(Application.dataPath + "/Res/Data/", relativePath);
    ...
    public static string[] GetConfigRawDatas(string fileName) {
        ...
        var content = ReadAllText(relativePath);
    ...
    private static void ReloadRuntimeConfig(string filePath) {
        ...
        var assembly = typeof(ConfigFileOpenUtils).Assembly;
        ...
        if (inited) {
            ...
            var lines = GetConfigRawDatas(fileName);
    ...
    private static void SetConfigFieldValue(object instance, FieldInfo field, string value) {
        ...
        switch (fieldType.Name.ToLower()) {
            ...
            case "int32[]":
                ...
                var intArray = new float[intStringArray.Length];
                for (int i = 0; i < intStringArray.Length; i++) {
                    float.TryParse(intStringArray[i], out intArray[i]);
                }
    ...
    private static DataTable GetExcelData(string excelFilePath) {
        ...
        using (var fs = new FileStream(excelFilePath, FileMode.Open, FileAccess.Read)) {
            ...
            var colCount = sheet.GetRow(2).LastCellNum;
